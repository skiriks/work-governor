...
- the current workflow phase, plan's actual status (proposed, approved for execution, or plan-only), exact remaining step, settled decisions and assumptions; for a child task, distinguish the approved parent strategy from approval of the child's own implementation;
...
End a completed coordination-only request with the requested proposal or handoff and its status. In a governed handoff reply, follow the brief with the source task's next action: if dispatch was not authorized, tell the user to supply/adopt the brief in the destination task; after authorized dispatch, name the actual wait or integration action. Do not imply that an uncreated destination is already executing. If the ongoing plan is blocked, include in the user-visible reply itself why, the precise user action and expected evidence, feasible alternatives with a recommendation, and what will resume afterward; internal status fields are not a substitute. If no user action is needed for the completed scope, say so. Do not stop executable plan work merely to announce that a coordination step ended.
